...
function EFFECT:Init( data )
	...
	self.TracerTime = math.min( 1, self.StartPos:Distance( self.EndPos ) / 4000 ) *0.2
	...
	for i = 0, 12 do
		local Pos = self.StartPos + Dir * i * 0.7 * math.random(1,2) * 0.2
		...
		if particle then
			...
			particle:SetStartSize( math.max( math.random(10,24) - i * 0.2,0.1 ) * Size )
			particle:SetEndSize( 0 )
			particle:SetRoll( math.Rand( -1, 1 ) )
			particle:SetColor( 255,174,0 )
...
function EFFECT:Think()
	...
	if CurTime() > self.DieTime then
		local effectdata = EffectData()
			effectdata:SetStart( Vector(255,174,0) ) 
