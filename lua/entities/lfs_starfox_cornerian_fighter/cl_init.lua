...
function ENT:AnimLandingGear()
	if !self:GetEngineActive() && self:GetLGear() > 0 then
		move = LerpAngle(3 *FrameTime(),move,Angle(0,0,90))
	else
		move = LerpAngle(3 *FrameTime(),move,Angle(0,0,0))
	end
	self:ManipulateBoneAngles(1,move)
end